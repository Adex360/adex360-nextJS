...
import { ClipboardList, Search, Send, Target } from "lucide-react";
...
const steps = [
  {
    ...
    title: "Design",
    description:
      "We map out your app's structure, wireframes and UI so every screen has a clear purpose before development begins.",
    offset: "lg:mt-0",
    badge: "01",
  ...
  },
  {
    ...
    title: "Develop",
    description:
      "Custom features, integrations and Shopify APIs come together — built clean, tested thoroughly, and reviewed line by line.",
    offset: "lg:mt-24",
    badge: "02",
  ...
  },
  {
    ...
    title: "Deliver",
    description:
      "We launch, audit and fine-tune your app for speed and stability, so it keeps performing long after it ships.",
    offset: "lg:mt-6",
    badge: "03",
    badgePos: "-right-1 bottom-1",
  },
...
export default function ShopifyProcess() {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      const maskPath = root.querySelector<SVGPathElement>("[data-sp-mask-path]");
      if (maskPath) {
      ...
      tl.fromTo("[data-sp-eyebrow]", { y: 16, opacity: 0 }, { y: 0, opacity: 1, duration: 0.5 })
        .fromTo(
          "[data-sp-heading]",
          { yPercent: 115 },
      ...
      gsap.utils.toArray<HTMLElement>("[data-sp-step]", root).forEach((el, i) => {
        const at = `path+=${0.15 + i * 0.6}`;
        tl.fromTo(el, { y: 44, opacity: 0 }, { y: 0, opacity: 1, duration: 0.7 }, at).fromTo(
          el.querySelector("[data-sp-badge]"),
          { scale: 0 },
    ...
    <section className="relative overflow-hidden bg-white px-4 py-16 sm:px-6 md:py-24 lg:px-8">
      <div ref={rootRef} className="relative mx-auto max-w-7xl">
        <div className="text-center">
          <p data-sp-eyebrow="" className="text-xs font-semibold uppercase tracking-widest text-brand-blue">
            Work Flow
          </p>
          <h2 className="mt-3 overflow-hidden pb-1 text-2xl font-extrabold text-ink sm:text-3xl lg:text-4xl">
            <span data-sp-heading="" className="block">
        ...
        <div className="relative mt-16 lg:mt-10">
          ...
          >
            <defs>
              <linearGradient id="spPathGrad" gradientUnits="userSpaceOnUse" x1="0" y1="0" x2="1200" y2="0">
            ...
            <mask id="spPathMask" maskUnits="userSpaceOnUse">
              <path
                data-sp-mask-path=""
                d={PATH_D}
                fill="none"
