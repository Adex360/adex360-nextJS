...
import { NotebookPen, Rocket, Send, TrendingUp } from "lucide-react";
...
const steps = [
  {
    ...
    title: "Strategize",
    description:
      "We study your audience, competitors and channels to build a data-backed performance media plan.",
    offset: "lg:mt-0",
    badge: "01",
  ...
  },
  {
    ...
    title: "Execute",
    description:
      "PPC, affiliate, email and display campaigns launch across the platforms that matter to your buyers.",
    offset: "lg:mt-24",
    badge: "02",
  ...
  },
  {
    ...
    title: "Grow",
    description:
      "We track ROAS, refine targeting and scale what's converting to compound your ad performance.",
    offset: "lg:mt-6",
    badge: "03",
    badgePos: "-right-1 bottom-1",
  },
...
export default function PerformanceProcess() {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      const maskPath = root.querySelector<SVGPathElement>("[data-pp-mask-path]");
      if (maskPath) {
      ...
      tl.fromTo("[data-pp-eyebrow]", { y: 16, opacity: 0 }, { y: 0, opacity: 1, duration: 0.5 })
        .fromTo(
          "[data-pp-heading]",
          { yPercent: 115 },
      ...
      gsap.utils.toArray<HTMLElement>("[data-pp-step]", root).forEach((el, i) => {
        const at = `path+=${0.15 + i * 0.6}`;
        tl.fromTo(el, { y: 44, opacity: 0 }, { y: 0, opacity: 1, duration: 0.7 }, at).fromTo(
          el.querySelector("[data-pp-badge]"),
          { scale: 0 },
    ...
    <section className="relative overflow-hidden bg-white px-4 py-16 sm:px-6 md:py-24 lg:px-8">
      <div ref={rootRef} className="relative mx-auto max-w-7xl">
        <div className="text-center">
          <p data-pp-eyebrow="" className="text-xs font-semibold uppercase tracking-widest text-brand-blue">
            Work Flow
          </p>
          <h2 className="mt-3 overflow-hidden pb-1 text-2xl font-extrabold text-ink sm:text-3xl lg:text-4xl">
            <span data-pp-heading="" className="block">
              Our Performance Marketing Operation
            </span>
          </h2>
        </div>
        ...
        <div className="relative mt-16 lg:mt-10">
          ...
          >
            <defs>
              <linearGradient id="ppPathGrad" gradientUnits="userSpaceOnUse" x1="0" y1="0" x2="1200" y2="0">
            ...
            <mask id="ppPathMask" maskUnits="userSpaceOnUse">
              <path
                data-pp-mask-path=""
                d={PATH_D}
                fill="none"
