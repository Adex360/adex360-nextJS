...
const steps = [
  {
    ...
    title: "Assessment",
    description:
      "We audit your current workflows, tools and data structure to map out exactly what your CRM needs to do.",
    offset: "lg:mt-0",
    badge: "01",
  ...
  },
  {
    ...
    description:
      "We build and connect your GoHighLevel CRM — pipelines, automations and integrations — tested end to end.",
    offset: "lg:mt-24",
    badge: "02",
  ...
  },
  {
    ...
    title: "Optimization",
    description:
      "We monitor performance and refine workflows continuously, so your CRM keeps getting sharper over time.",
    offset: "lg:mt-6",
    badge: "03",
    badgePos: "-right-1 bottom-1",
  },
...
export default function CrmProcess() {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      const maskPath = root.querySelector<SVGPathElement>("[data-cp-mask-path]");
      if (maskPath) {
      ...
      tl.fromTo("[data-cp-eyebrow]", { y: 16, opacity: 0 }, { y: 0, opacity: 1, duration: 0.5 })
        .fromTo(
          "[data-cp-heading]",
          { yPercent: 115 },
      ...
      gsap.utils.toArray<HTMLElement>("[data-cp-step]", root).forEach((el, i) => {
        const at = `path+=${0.15 + i * 0.6}`;
        tl.fromTo(el, { y: 44, opacity: 0 }, { y: 0, opacity: 1, duration: 0.7 }, at).fromTo(
          el.querySelector("[data-cp-badge]"),
          { scale: 0 },
    ...
    <section className="relative overflow-hidden bg-white px-4 py-16 sm:px-6 md:py-24 lg:px-8">
      <div ref={rootRef} className="relative mx-auto max-w-7xl">
        <div className="text-center">
          <p data-cp-eyebrow="" className="text-xs font-semibold uppercase tracking-widest text-brand-blue">
            Work Flow
          </p>
          <h2 className="mt-3 overflow-hidden pb-1 text-2xl font-extrabold text-ink sm:text-3xl lg:text-4xl">
            <span data-cp-heading="" className="block">
              Our CRM Integration Operation
            </span>
          </h2>
        </div>
        ...
        <div className="relative mt-16 lg:mt-10">
          ...
          >
            <defs>
              <linearGradient id="cpPathGrad" gradientUnits="userSpaceOnUse" x1="0" y1="0" x2="1200" y2="0">
            ...
            <mask id="cpPathMask" maskUnits="userSpaceOnUse">
              <path
                data-cp-mask-path=""
                d={PATH_D}
                fill="none"
