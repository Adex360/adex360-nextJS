...
**Last updated:** August 20, 2026 (Contact form swapped for the real GHL booking iframe + cross-origin padding fix; hero blob shape-cycling standardized)
...
- [ ] SEO-score form (`SeoScoreCta.tsx`) still needs API route + email delivery, or a GHL form embed of its own if the user provides one
...
1. **Remaining service pages** — `/web-development`, `/shopify-app-development`, `/crm-integration`. Reuse the Performance/SEO/Social Media pages as the template (hero, tabs/feature sections, process path, testimonials, FAQ). Blocked on user input per page: WP screenshots + inspector HTML for tabs/testimonials/FAQ + CTA link targets.
1a. **Confirm 3 flagged Performance Marketing content blocks** (Unique paragraph/CTA, Growth eyebrow/paragraph, Advantage's 8 bullets) — sent to user for closer screenshots; low-priority polish, page already ships.
